...
install_packages() {
	...
	local utils='aspell bash-completion ddcutil hunspell nano pacman-contrib'
	utils+=' perl-rename sudo'
	...
	# gnome
	# 
	# dunst blueman copyq hyprland hypridle hyprlock hyprpolkitagent kitty
	# network-manager-applet rofi waybar wpaperd xdg-desktop-portal-gtk  
...
setup_files() {
	...
	/* Allow users in wheel group to use blueman without authentication */
	...
	echo 'i2c-dev' > "$1/etc/modules-load.d/i2cdev.conf"
}
